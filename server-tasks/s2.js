...
// var URL = "https://bus-v2-0128.firebaseio.com/";
...
console.log("node-crontab .... for every 30 min ");
...
fbRef.authWithCustomToken(token, function (error, authData) {
    if (error) {
        ...
        var jobId = crontab.scheduleJob("*/30 * * * *", function(){ //This will call this function every 2 minutes
            console.log("It's been 30 minutes!--------##### "+Date());
            fetch_one_set_and_show_json_problem(get_option(urlSrc));
            ...
            if (encoding && encoding.indexOf('gzip') >= 0) {
                zlib.gunzip(body, function (err, dezipped) {
                    ...
                    try {
                        ...
                        var routesv5OnComplete = function(error) {
                          if (error) {
                            console.log('routesv5OnComplete Synchronization failed'+error );
                          } else {
                            console.log('routesv5OnComplete Synchronization succeeded');
                          }
                        };
                        ref.child('routesv5').set({routes:docs},routesv5OnComplete);
