...
var token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ2IjowLCJkIjp7InVpZCI6Im1hcmsiLCJzb21lIjoiYXJiaXRyYXJ5IiwiZGF0YSI6ImhlcmUifSwiaWF0IjoxNDU0MjQ0NDk5fQ.pKQaNbybfFJwp8tqZoh7CBSyUULqmSoXFrL-uC1Z0HE";
...
console.log("node-crontab .... per min testing");
...
fbRef.authWithCustomToken(token, function (error, authData) {
    if (error) {
        ...
        var jobId = crontab.scheduleJob("* * * * *", function(){ //This will call this function every 2 minutes
            console.log("It's been 1 minutes!--------"+Date());
            fetch_one_set_and_show_json_problem(get_option(urlSrc));
            ...
            if (encoding && encoding.indexOf('gzip') >= 0) {
                zlib.gunzip(body, function (err, dezipped) {
                    ...
                    try {
                        ...
                        var onDocNumComplete = function(error) {
                          if (error) {
                            console.log('buslistv8-docnum:'+docNum+', onSetComplete Synchronization failed'+error);
                          } else {
                            console.log('buslistv8-docnum:'+docNum+', onSetComplete Synchronization succeeded');
                        ...
                        var onSetComplete = function(error) {
                          if (error) {
                            console.log('buslistv8-doc'+docNum+"/doc"+', onSetComplete Synchronization failed'+error);
                          } else {
                            console.log('buslistv8-doc'+docNum+"/doc"+', onSetComplete Synchronization succeeded');

                            fbRef.child('buslistv8-docnum').set({num:docNum}, onDocNumComplete );
                          }
                        };

                        // 2/4 18:24
                        fbRef.child('buslistv8-doc'+docNum+"/doc").set(busList3,onSetComplete ); // 只要有出現的公車就加進來
